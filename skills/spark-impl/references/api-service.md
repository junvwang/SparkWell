...
- Otherwise implement the smallest compatible handlers and adapters within the confirmed project architecture.

## Architecture Boundary

- Resolve service architecture, module boundaries, domain and boundary mappings, persistence access, repository and dependency-injection patterns, transaction handling, and local or remote dependencies from the selected profile, its guidance, and an established native project.
- This target reference does not choose Clean Architecture, layered architecture, repository patterns, ORMs, persistence providers, synchronization strategies, or source layout.
- Preserve an established API architecture. For a new implementation, mark unresolved consequential choices **Blocked** and direct the user to `/spark-config`.
