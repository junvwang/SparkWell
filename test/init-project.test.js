...
test('core project templates preserve the methodology quality contract', async () => {
  ...
  assert.match(usage, /## Project Setup Flow/)
  assert.match(usage, /Run `sparkwell init` with the required coding-agent adapters and optional Packs/)
  assert.match(usage, /Review `\.sparkwell\/design-context\.md` and record durable project-wide facts that may affect Spark boundaries/)
  assert.match(usage, /Design can begin before implementation profiles exist/)
  assert.match(usage, /Before the first `\/spark-impl` for a target, copy the commented profile example in `\.sparkwell\/config\.yaml`/)
  assert.match(usage, /create its referenced `\.sparkwell\/guidance\/<profile-id>\.md` files/)
  assert.match(usage, /The initializer supplies placeholders and directories, not project decisions/)
  assert.match(usage, /Normal reinitialization preserves them/)
